                ...
                <tr v-for="cat in store.categories" :key="cat.id" class="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td class="px-6 py-4">
                    <div class="flex items-center gap-2">
                      <div class="w-6 h-6 rounded-full bg-slate-900 flex items-center justify-center text-white text-[9px] font-black uppercase flex-shrink-0">
                        {{ (cat.creator_name || '?').charAt(0) }}
                      </div>
                      <div>
                        <p class="text-xs font-bold text-slate-700">{{ cat.creator_name || 'Tidak diketahui' }}</p>
                        <span v-if="cat.user_id == user.id" class="text-[9px] font-black text-blue-500 bg-blue-50 px-1.5 py-0.5 rounded">Anda</span>
                      </div>
                    </div>
                  </td>
                  ...
                  <td class="px-6 py-4 text-right">
                    <div class="flex justify-end items-center gap-2">
                      ...
                      <button 
                        @click="router.push(`/admin/articles/create?category_id=${cat.id}`)" 
                      ...
                      <button 
                        @click="router.push(`/admin/gallery/tambah?category_id=${cat.id}`)" 
...
const user = ref({ name: 'Admin', email: '', id: null });
