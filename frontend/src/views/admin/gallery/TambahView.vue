...
const handleSubmit = async () => {
    ...
    // Sesuai dengan metode bulkStore: images[]
            ...
            <main class="p-6">
                <div class="max-w-4xl mx-auto">
                    ...
                    <form @submit.prevent="handleSubmit" class="bg-white rounded-3xl p-8 border border-slate-200 shadow-sm space-y-6">
                        ...
                        <div>
                            <label class="block text-sm font-black text-slate-700 uppercase tracking-wider mb-2">
                                Pilih Gambar (Maksimal 10)
                            </label>
                            <div class="border-2 border-dashed border-slate-200 rounded-2xl p-8 text-center hover:bg-slate-50 transition-all cursor-pointer relative"
                                :class="{'border-red-400 bg-red-50': form.images.length > 10}">
                                ...
                                <div class="text-slate-400">
                                    ...
                                    <p class="font-bold">Terpilih: {{ form.images.length }} / 10 Gambar</p>
                                    <p class="text-xs">PNG, JPG, WEBP (Max 2MB per file)</p>
                                </div>
                            </div>
                            <p v-if="errors.images" class="text-red-500 text-xs mt-1">{{ errors.images[0] }}</p>
                        </div>

                        <div v-if="previews.length > 0" class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-4">
                            <div v-for="(src, index) in previews" :key="index" class="relative aspect-square rounded-xl overflow-hidden border border-slate-100 shadow-sm">
