...
const fetchEmployees = async (page = 1) => {
    ...
    try {
        const response = await Api.get('/admin/employees', { params: { page } })
...
const submitEmployee = async () => {
    ...
    try {
        const response = await Api.post('/admin/employees', form.value)
                                    ...
                                    <tr v-for="employee in employees" :key="employee.id" class="hover:bg-slate-50">
                                        ...
                                        <td class="px-4 py-4 text-right">
                                            <div class="inline-flex gap-2 justify-end">
                                                <button @click="router.push(`/admin/karyawan/edit/${employee.id}`)" class="inline-flex items-center gap-2 text-slate-700 hover:bg-slate-100 border border-slate-200 rounded-xl px-3 py-2 text-xs font-bold transition-all">
                                                    Edit
                                                </button>
