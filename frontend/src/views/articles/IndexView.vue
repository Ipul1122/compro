...
import Api from '@/api' // Menggunakan instance axios yang sudah Anda buat
import Navbar from '@/components/Navbar.vue' // Navbar publik
...
const fetchArticles = async (page = 1) => {
    ...
    try {
        const params = {
            ...
            published: 'publish' // Hanya tampilkan yang sudah dipublish untuk publik
        }

        // Memanggil API articles menggunakan instance Api
        ...
        if (response.data.pagination) {
            ...
            router.push({ path: '/articles', query: queryParams })
        }
    } catch (error) {
    ...
    <div class="min-h-screen bg-slate-50 flex flex-col">
        ...
        <main class="flex-grow container mx-auto px-4 py-8 max-w-7xl">
            <div class="mb-10 text-center md:text-left">
                <h1 class="text-3xl font-black text-slate-900 uppercase tracking-tight mb-2">Artikel Terbaru</h1>
                <p class="text-slate-500 mb-8 font-medium">Temukan wawasan dan berita terbaru dari kami.</p>

                <div class="bg-white rounded-3xl border border-slate-100 p-4 shadow-sm flex flex-col md:flex-row gap-4">
                    <div class="flex-1">
                    ...
                    <div class="md:w-64">
                        <select v-model="filters.category_id" @change="applyFilters" 
                            class="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-3 text-sm focus:outline-none cursor-pointer">
                ...
                <div v-for="article in articles" :key="article.id" class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden hover:shadow-xl hover:shadow-slate-200/50 transition-all group flex flex-col">
                    ...
                    <div class="p-6 flex flex-col flex-1">
                        ...
                        <div class="mt-auto pt-5 border-t border-slate-50 flex justify-between items-center">
                            <router-link :to="`/articles/${article.id}`" class="text-sm font-black text-slate-900 uppercase tracking-wider flex items-center gap-2 group/link">
                                Selengkapnya
            ...
            <div v-if="lastPage > 1" class="flex justify-center items-center mt-12 gap-2">
                <button @click="fetchArticles(currentPage - 1)" :disabled="currentPage === 1"
                    class="p-2 rounded-xl bg-white border border-slate-100 disabled:opacity-50 cursor-pointer hover:bg-slate-50">
                ...
                <button @click="fetchArticles(currentPage + 1)" :disabled="currentPage === lastPage"
                    class="p-2 rounded-xl bg-white border border-slate-100 disabled:opacity-50 cursor-pointer hover:bg-slate-50">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
