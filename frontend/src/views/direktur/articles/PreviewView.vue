        ...
        <div class="max-w-4xl mx-auto mb-6">
          <div class="bg-amber-500 text-white p-4 rounded-2xl flex justify-between items-center shadow-lg shadow-amber-100">
            <div class="flex items-center gap-3">
              ...
              <span class="font-bold">Pratinjau Artikel: Tinjau sebelum dipublish.</span>
            </div>
          </div>
        </div>
        ...
        <article v-else-if="article" class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden max-w-4xl mx-auto">
          ...
          <div class="p-6 md:p-12">
            ...
            <div class="mt-8 flex justify-end gap-3">
              ...
              <button @click="handlePublish" :disabled="isUpdating" class="bg-amber-500 text-white px-8 py-3 rounded-xl text-sm font-bold hover:bg-amber-600 transition-colors cursor-pointer disabled:opacity-50 flex items-center justify-center min-w-[160px]">
                {{ isUpdating ? 'Memproses...' : 'Publish Artikel' }}
              </button>
            </div>
          </div>
        </article>
...
const fetchArticle = async () => {
    loading.value = true;
    try {
        // Menggunakan endpoint yang sama seperti admin preview (akses berdasarkan slug)
...
const handlePublish = async () => {
    ...
    try {
        ...
        console.error("Gagal mempublish artikel:", error);
        alert('Terjadi kesalahan saat mempublish artikel.');
    } finally {
        isUpdating.value = false;
    }
};
