        ...
        <div class="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm max-w-5xl mx-auto">
          ...
          <form v-else @submit.prevent="confirmUpdate" enctype="multipart/form-data">
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-slate-100">
              <router-link to="/direktur/articles" class="bg-slate-100 text-slate-600 px-8 py-3 rounded-xl text-sm font-bold hover:bg-slate-200 transition-colors text-center cursor-pointer">Kembali</router-link>
...
const fetchArticleDetail = async () => {
    ...
    try {
        const response = await Api.get(`/direktur/articles/${route.params.id}`);
...
const executeSubmit = async () => {
    ...
    try {
        ...
        await Api.post(`/direktur/articles/${route.params.id}`, formData, {
            headers: { 
...
const handleNavigation = (view) => {
    if (view === 'dashboard') router.push('/direktur/dashboard');
    else if (view === 'categories') router.push('/direktur/categories');
};
