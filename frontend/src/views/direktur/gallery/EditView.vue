...
const breadcrumbsData = ref([
    ...
    { label: 'Manage Gallery', link: '/direktur/gallery' },
...
const fetchData = async () => {
    ...
    try {
        const catRes = await Api.get('/direktur/categories/list')
        categories.value = catRes.data.data || catRes.data

        const galRes = await Api.get('/direktur/galleries')
...
const handleSubmit = async () => {
    ...
    try {
        ...
        const updatePromises = albumItems.value.map((item, index) => {
            ...
            return Api.post(`/direktur/galleries/${item.id}`, formData)
        })
        ...
        if (form.value.new_images.length > 0) {
            ...
            await Api.post('/direktur/galleries/bulk-store', bulkData)
        }
            ...
            <main class="p-6">
                <div class="max-w-6xl mx-auto">
                    <div class="mb-6 flex justify-between items-end">
                        <div>
                            <router-link to="/direktur/gallery" class="text-slate-400 hover:text-slate-600 text-sm font-bold flex items-center gap-2 mb-2">
