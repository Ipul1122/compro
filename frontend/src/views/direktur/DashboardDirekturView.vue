...
const fetchArticlesStats = async () => {
    try { const r = await Api.get('/admin/articles'); articles.value = r.data.data || r.data } catch (e) { console.error(e) }
}
const fetchCategoriesStats = async () => {
    try { const r = await Api.get('/admin/categories'); categories.value = r.data.data || r.data } catch (e) { console.error(e) }
}
const fetchGalleriesStats = async () => {
    try { const r = await Api.get('/admin/galleries'); galleries.value = r.data.data || r.data } catch (e) { console.error(e) }
}
...
const statCards = computed(() => [
    { label: 'Kategori', value: categories.value.length, route: '/admin/categories', icon: 'M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z', color: 'violet' },
    { label: 'Artikel', value: articles.value.length, route: '/admin/articles', icon: 'M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9.5a2.5 2.5 0 00-2.5-2.5H14', color: 'sky' },
    { label: 'Galeri', value: galleries.value.length, route: '/admin/gallery', icon: 'M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z', color: 'amber' }
])
</script>
        ...
        <div class="flex-1 flex flex-col min-w-0">
            ...
            <main class="p-4 md:p-6 lg:p-8 space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <router-link to="/admin/articles/create" class="quick-action group flex items-center gap-4 bg-white rounded-2xl border border-slate-100 p-4 shadow-sm hover:shadow-md hover:border-emerald-200 transition-all duration-300">
                    ...
                    <router-link to="/admin/categories" class="quick-action group flex items-center gap-4 bg-white rounded-2xl border border-slate-100 p-4 shadow-sm hover:shadow-md hover:border-violet-200 transition-all duration-300">
                    ...
                    <router-link to="/admin/gallery" class="quick-action group flex items-center gap-4 bg-white rounded-2xl border border-slate-100 p-4 shadow-sm hover:shadow-md hover:border-amber-200 transition-all duration-300">
