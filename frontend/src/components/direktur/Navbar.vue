...
const displayedNotificationCount = computed(() => {
    ...
    if (typeof props.notificationCount === 'number') {
...
const fetchLocalNotifications = async () => {
    try {
        ...
        try {
            const r = await Api.get('/direktur/articles')
            const items = r.data?.data || []
            localNotifications.value = items.slice(0, 8).map((article) => ({
                id: article.id,
                title: article.title,
                message: `${article.author?.name || 'Tanpa Nama'} membuat artikel: ${article.title}`,
                created_at_human: article.created_at ? new Date(article.created_at).toLocaleString('id-ID') : '-',
                is_read: true,
            }))
            localUnreadCount.value = 0
        } catch (fallbackError) {
            console.error('Notif fetch error:', fallbackError)
        }
    }
}
                ...
                <transition name="pop">
                    <div v-if="isNotifOpen" class="absolute right-0 mt-3 w-80 bg-white rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-slate-100 overflow-hidden z-[120]">
                        <div class="px-4 py-3 border-b border-slate-100">
                            ...
                            <p class="text-[11px] text-slate-400 mt-1">Aktivitas terbaru artikel dan galeri.</p>
                        </div>
